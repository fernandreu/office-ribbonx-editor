...
public class SampleTests
{
    ...
    {
        Assert.DoesNotThrow(() => SampleUtils.LoadXmlSamples(["A", "B", "C"], false));
    }
    ...
    {
        // Act
        var result = SampleUtils.LoadXmlSamples([], true);
            
        // Assert
        ScanResult(result!, x => Assert.That(x is FileSampleViewModel, Is.False));
    ...
    public static void CanScanSingleFiles()
    {
        ...
        // Act
        var result = SampleUtils.LoadXmlSamples([path], false);

        // Assert
        try
        {
            Assert.That(result!.Items.Count, Is.EqualTo(1));
            Assert.That(result.Items[0].Name, Is.EqualTo(name));
        }
        finally
        {
            File.Delete(path);
    ...
    public static void CanScanFolder(int numFiles, int numFolders, int nestedLevels)
    {
        ...
        // Act
        var result = SampleUtils.LoadXmlSamples([root], false);
    ...
    private static void AssertConsistency(SampleFolderViewModel expected, SampleFolderViewModel actual, string? path = null)
    {
        ...
        foreach (var item in expected.Items)
        {
            if (item is SampleFolderViewModel folder)
            {
                ...
                Assert.That(other, Is.Not.Null, $"Could not find folder {path}/{item.Name}");
                AssertConsistency(folder, other!, path);
            }
            else
            {
                var other = actual.Items.OfType<FileSampleViewModel>().FirstOrDefault(x => x.Name == item.Name);
                Assert.That(other, Is.Not.Null, $"Could not find file {path}/{item.Name}");
            }
        }
    }
}
...
#pragma warning restore CS8618 // Non-nullable field is uninitialized. Consider declaring as nullable.
    ...
    public void TestCut()
    {
        ...
        // Assert
        Assert.That(Clipboard.GetText(), Is.EqualTo(_originalCode));
    ...
    public void TestCopy()
    {
        ...
        // Assert
        Assert.That(Clipboard.GetText(), Is.EqualTo(_editor.Text));
    }
    ...
    public void TestPaste()
    {
        ...
        // Assert
        Assert.That(_editor.Text, Is.Not.EqualTo(_originalCode));
    ...
    public void TestFoldingLevels()
    {
        ...
        for (var level = 0; level <= 8; ++level)
        {
            ...
            Assert.That(_editor.Text, Is.EqualTo(_originalCode));
        }
    }
    ...
    public void TestGoTo(int originalLine, int newLine, int expected)
    {
        ...
        Assert.That(dialog, Is.Not.Null,"No dialog launched");

        var currentLineBox = dialog!.FindFirstDescendant("CurrentLineBox").AsTextBox();
        Assert.That(currentLineBox?.Text, Is.EqualTo($"{originalLine}"));

        var maximumLineBox = dialog.FindFirstDescendant("MaximumLineBox").AsTextBox();
        Assert.That(maximumLineBox?.Text, Is.EqualTo($"{textParts.Length}"));
        ...
        // Assert
        Assert.That(_editor.Selection.Line + 1, Is.EqualTo(expected));
    }
